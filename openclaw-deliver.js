...
import {
  ...
  u as logVerbose,
} from "./subsystem-nlluZawe.js";
import {
  h as normalizeOptionalAccountId,
  m as normalizeAccountId,
} from "./session-key-a6av96Fj.js";
import {
  a as triggerInternalHook,
  t as createInternalHookEvent,
} from "./internal-hooks-Y1c3CR6c.js";
import {
  c as getActivePluginRegistry,
  u as getActivePluginRegistryVersion,
} from "./registry-ycXZ0GNV.js";
import { t as getChannelDock } from "./dock-D49biwXh.js";
import {
  i as isSilentReplyText,
  n as SILENT_REPLY_TOKEN,
} from "./tokens-BXGWtt7T.js";
import {
  r as normalizeChannelId,
  t as getChannelPlugin,
} from "./plugins-noDEKrDH.js";
...
import {
  i as resolveMirroredTranscriptText,
  r as appendAssistantMessageToSessionTranscript,
  w as parseInlineDirectives,
} from "./sessions-_Cu6BqJv.js";
import { i as getAgentScopedMediaLocalRoots } from "./fetch-DJLxHaLt.js";
import {
  c as resolveChunkMode,
  f as parseFenceSpans,
  i as chunkMarkdownTextWithMode,
  l as resolveTextChunkLimit,
  n as chunkByParagraph,
} from "./chunk-Cgy4C3Nh.js";
import { n as resolveMarkdownTableMode } from "./markdown-tables-KowmfLDF.js";
import {
  l as markdownToSignalTextChunks,
  t as sendMessageSignal,
} from "./send-Cwk6REJC.js";
...
function initializeGlobalHookRunner(registry) {
  ...
  if (hookCount > 0)
    log$1.info(`hook runner initialized with ${hookCount} registered hooks`);
}
/**
...
function normalizeTargetForProvider(provider, raw) {
  ...
  return (
    ((providerId ? resolveTargetNormalizer(providerId) : void 0)?.(raw) ??
      fallback) ||
    void 0
  );
}
...
function resolveReplyToMode(cfg, channel, accountId, chatType) {
  ...
  return (
    getChannelDock(provider)?.threading?.resolveReplyToMode?.({
      cfg,
      accountId,
      chatType,
    }) ?? "all"
...
function createReplyToModeFilterForChannel(mode, channel) {
  const provider = normalizeChannelId(channel);
  const isWebchat =
    (typeof channel === "string" ? channel.trim().toLowerCase() : void 0) ===
    "webchat";
  const dock = provider ? getChannelDock(provider) : void 0;
  return createReplyToModeFilter(mode, {
    allowExplicitReplyTagsWhenOff: provider
...
function shouldSuppressMessagingToolReplies(params) {
  ...
  return sentTargets.some((target) => {
    ...
    const targetKey = normalizeTargetForProvider(
      isGenericMessageProvider ? provider : targetProvider,
      target.to,
    );
  ...
  for (const line of lines) {
    ...
    for (const match of matches) {
      ...
      for (const part of parts) {
        ...
        if (
          isValidMedia(candidate, unwrapped ? { allowSpaces: true } : void 0)
        ) {
...
function normalizeReplyPayloadsForDelivery(payloads) {
  ...
  for (const payload of payloads) {
    ...
    const resolvedMediaUrl =
      (explicitMediaUrls?.length ?? 0) > 1 ? void 0 : explicitMediaUrl;
    const next = {
      ...payload,
      text: parsed.text ?? "",
...
function normalizeOutboundPayloadsForJson(payloads) {
  ...
  for (const payload of normalizeReplyPayloadsForDelivery(payloads))
    normalized.push({
      ...
      mediaUrls:
        payload.mediaUrls ?? (payload.mediaUrl ? [payload.mediaUrl] : void 0),
...
var deliver_exports = /* @__PURE__ */ __exportAll({
...
function normalizePayloadsForChannelDelivery(payloads, channel) {
  ...
  for (const payload of normalizeReplyPayloadsForDelivery(payloads)) {
    ...
    const normalized = normalizePayloadForChannelDelivery(
      sanitizedPayload,
      channel,
    );
...
async function deliverOutboundPayloadsCore(params) {
  ...
  const sendTextChunks = async (text, overrides) => {
    ...
    if (chunkMode === "newline") {
      const blockChunks =
        (handler.chunkerMode ?? "text") === "markdown"
...
//#endregion
export {
  initializeGlobalHookRunner as A,
  shouldSuppressReasoningPayload as C,
  normalizeChannelTargetInput as D,
  buildTargetResolverSignature as E,
  normalizeTargetForProvider as O,
  shouldSuppressMessagingToolReplies as S,
  resolveReplyToMode as T,
  applyReplyTagsToPayload as _,
  normalizeOutboundPayloadsForJson as a,
  filterMessagingToolMediaDuplicates as b,
  parseReplyDirectives as c,
  toInternalMessagePreprocessedContext as d,
  toInternalMessageReceivedContext as f,
  fireAndForgetHook as g,
  toPluginMessageReceivedEvent as h,
  normalizeOutboundPayloads as i,
  getGlobalHookRunner as k,
  splitMediaFromOutput as l,
  toPluginMessageContext as m,
  deliver_exports as n,
  normalizeReplyPayloadsForDelivery as o,
  toInternalMessageTranscribedContext as p,
  formatOutboundPayloadLog as r,
  throwIfAborted as s,
  deliverOutboundPayloads as t,
  deriveInboundMessageHookContext as u,
  applyReplyThreading as v,
  createReplyToModeFilterForChannel as w,
  isRenderablePayload as x,
  filterMessagingToolDuplicates as y,
};
